...
```{r}
adm %>%
  mutate(test = ifelse((is.na(deathtime)==F & 
...
```{r}
labevents_icu <- fread("labevents_icu.csv.gz",
                         nThread = getDTthreads())
labevents_icu
```
...
**Check the content of the processed data**
```{r}
chartevents_icu <- fread("chartevents_icu.csv.gz",
                         nThread = getDTthreads())
chartevents_icu
```
...
We found that there are some patients have more than one record at a single time point. Thus, we need to only keep one record.
...
labevents_icu %>%
  ...
  slice_min(charttime) %>%
  slice(1) %>%
  select(-c(charttime,itemid)) %>%
  spread(key = label, value = valuenum) -> labevents_icu
...
chartevents_icu %>%
  ...
  slice(1) %>%
  select(-c(charttime,itemid)) %>%
  spread(key = label, value = valuenum) -> chartevents_icu
```
...
icustays %>%
  ...
  mutate(death_30 = ifelse(deathtime - intime <= 30,
                           "Yes", "No")) -> final_icu_dataset
