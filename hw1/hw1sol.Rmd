...
> **solution**:
```{bash}
zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz | awk '(NR<=5)'
```
```{bash}
...
zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz | awk  -F ',' '{ print $1 }' | uniq | wc -l
```
...
cal 9 1752
```
`cal 9 1752` seems there some days missing in September 1752.
```{bash}
date
```
`date` seems there some days missing in September 1752.
```{bash}
hostname
```
...
uname -a
```
`uname -a` provides the computer architecture.
