...
v_t1_data <- eventReactive(input$v_t1_select, {
  
  x <- isolate(input$v_t1_value)
  y <- isolate(input$v_t1_sub)
  z <- isolate(input$v_t1_group)
...
output$v_t1_plot1 <- renderPlotly({
  df <- v_t1_data()
...
output$v_t1_plot2 <- renderPlotly({
  df <- v_t1_data()
  na.num <- which(is.na(df$type)==T) %>% length
