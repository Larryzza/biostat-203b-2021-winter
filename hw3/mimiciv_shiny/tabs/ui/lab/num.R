lab_num <- tabPanel(title = "Descriptive statistics", 
                    value = "lab_num",
                    column(width = 10,
                           column(width = 2,
                                  pickerInput(inputId = "lab_t4_type", 
                                              label = "Group by", 
                    ...
                    column(width = 2, style = "padding-top: 50px;",
                           actionBttn(inputId = "lab_t4_select", 
                                      label = "Select", 
                                      style = "fill", 
                                      color = "success", 
                                      icon = icon("check"), size = "sm") 
                    ),
                    conditionalPanel(condition = "input.lab_t4_select < 1",
                                     column(width = 12,
                                            br(), 
                                            wellPanel(
                    ...
                    column(width = 12,
                                  conditionalPanel(condition = "input.lab_t4_select > 0",
                                                   br(), hr(), br(),
                                                   HTML("<center><h1>Numerical Summaries</h1></center>"),
                                                   column(width = 12,
                                                          withSpinner(dataTableOutput("lab_t4_ns"), type = 6)
                                                   )           
                                  )
                    )
)