...
According to the confusion Matrix, we found that the two models have similar prediction accuracy. XGBoost model has a slightly higher overall accuracy (0.79) and MLP model predict better for death_30 cases (0.69). Both two models have a Balanced Accuracy at around 0.70.

We also plotted ROC curves for two models. ROC plots shows that two models have similar prediction pattern and AUC value.
```{r}
rocmlp<-roc(data.test.y %>% factor, 
            results2 %>% as.numeric, quiet = TRUE)
rocxgs<-roc(data.test.y %>% factor, 
            xgbpred, quiet = TRUE)
