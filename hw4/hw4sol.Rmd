...
Check the plot of top 15 important variables
```{r}
importance_sum <- readRDS("importance_sum.rds")
ggplot(data=importance_sum[1:15,],
...
Then we did a forward-stepwise variable selection according to the AIC value.
```{r}
...
Finally, trained the dataset (re-sampled) and predicted.
```{r}
temp <- cbind(data.train.y, 
                data.train.x[,feature_select]) %>% as.data.frame()
data.balanced <- ovun.sample(.data_Yes~., data = temp, p = 0.33, 
                                seed = 1996, method = "under")$data
...
confusionMatrix (xgbpred %>% as.factor(), data.test.y %>% as.factor())
```
...
rocmlp<-roc(data.test.y, results2)
